...
public class AllowUsage extends ListenerAdapter {
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if (args[0].equalsIgnoreCase("$allow")) {
			...
			if(args.length != 2) {
				event.getChannel().sendMessage(redX + "Invalid format: `$allow [word]`").queue();
				return;
			}
			...
			if (!list.contains(args[1])) {
				event.getChannel().sendMessage(redX + "You already allow the word: " + args[1]).queue();
				return;
			}
			...
			if (!first.getString("FilterType").equalsIgnoreCase("Custom")) {
				event.getChannel().sendMessage(warning + "Warning: your server's filter is set to `"
						...
						+ "`. This means the allowed word will not be allowed unless you run the command: `$filter custom`"
						+ warning).queue();
			}
